...
export const DataProvider = ({ children }) => {
  // async function fetchBonnData() {
  //   const response = await fetch("/api/eventsBonn");
  //   const data = await response.json();
  //   return data;
  // }

  // async function getBonnData() {
  //   try {
  //     const data = await fetchBonnData();
  //     return data;
  //   } catch (error) {
  ...
  const {
    data: eventsBonn,
    isLoading: isLoadingEventsBonn,
    error: errorEventsBonn,
    mutate: mutateEventsBonn,
  } = useSWR("/api/eventsBonn", fetcher, { refreshInterval: 2 * 60 * 1000 });
