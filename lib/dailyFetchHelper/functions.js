...
export async function fetchUniqueEvents(mergedCategories) {
  ...
  for (const obj of mergedCategories) {
    ...
    try {
      ...
      // The fetch result is an object - The items key contains an array with the event objects
      const items = result.items.map((item) => ({
        ...item,
        assignedCategoryId: obj.assignedCategory._id,
        assignedSubCategoryId: obj.assignedSubCategory?._id || null,
        originalCategory: obj.kategorie,
      }));
...
export function mergeCategories(
  ...
  ourCategories.forEach((category) => {
    category.subCategories.forEach((subCat) =>
      subCategoriesMap.set(subCat.title, subCat)
    );
  });
  ...
  const mergedCategoriesWithSubCategories = mergedCategories
    .map((cat) => {
      const relation = categoryRelation.find(
        (rel) => rel.category === cat.kategorie
      );
      if (relation && subCategoriesMap.has(relation.assignToSubCategory)) {
        const ourSubCat = subCategoriesMap.get(relation.assignToSubCategory);
        return {
          ...cat,
          assignedSubCategory: {
            _id: ourSubCat._id,
            title: ourSubCat.title,
            slug: ourSubCat.slug,
            imageSource: ourSubCat.imageSource,
            imageAlt: ourSubCat.imageAlt,
            parentCategory: ourSubCat.parentCategory,
          },
        };
      }
      return {
  ...
  return mergedCategoriesWithSubCategories;
}
